...
if (f === 'targetMode') {
                                     ...
                                     if (f === 'burstMode') {
                                      ...
                                      if (f === 'slow_amount') {
                                          ...
                                          return `<div class="field" style="display:flex; flex-direction:column; gap:4px;"><label>${prefix}Ralentización</label><div style="display:flex; gap:6px; align-items:center;"><input type="number" step="0.1" min="0" value="${m[f] || 0}" style="flex:3; min-width:80px;" onchange="config.enemyModels['${selectedEnemyId}'].mechanics[${idx}].slow_amount = parseFloat(this.value);"><select style="flex:1; min-width:70px; background:#0f172a; border:1px solid var(--accent); color:var(--accent); font-size:0.75rem; padding:4px; border-radius:4px; cursor:pointer;" onchange="config.enemyModels['${selectedEnemyId}'].mechanics[${idx}].slowIsPercentage = this.value === 'pct'; renderEnemyDetail();"><option value="fixed" ${!isPct ? 'selected' : ''}>Fijo</option><option value="pct" ${isPct ? 'selected' : ''}>%</option></select></div></div>`;
                                      }
