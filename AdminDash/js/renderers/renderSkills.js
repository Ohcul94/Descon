    ...
    for(let name in config.skillsData) {
        ...
        card.innerHTML = `
            ...
            <div class="form-grid">
                ...
                <div class="field"><label>Apuntar a otros</label><input type="checkbox" ${s.canTargetOthers ? 'checked' : ''} onchange="config.skillsData['${name}'].canTargetOthers = this.checked"></div>
                <div class="field"><label>Modo de apunte</label><select onchange="config.skillsData['${name}'].aimType = parseInt(this.value)"><option value="-1" ${s.aimType === undefined || s.aimType === -1 ? 'selected' : ''}>Auto</option><option value="0" ${s.aimType === 0 ? 'selected' : ''}>Direccional</option><option value="1" ${s.aimType === 1 ? 'selected' : ''}>Punto (click)</option><option value="2" ${s.aimType === 2 ? 'selected' : ''}>Área</option><option value="3" ${s.aimType === 3 ? 'selected' : ''}>Instantáneo (self)</option></select></div>
                ...
                ${s.slow_amount !== undefined ? `<div class="field"><label>Slow (0-1)</label><input type="number" step="0.05" min="0" max="0.95" value="${s.slow_amount}" onchange="config.skillsData['${name}'].slow_amount = parseFloat(this.value)"></div>` : ''}
