...
func _ready():
	...
	if NetworkManager:
		...
		NetworkManager.auth_error.connect(func(msg): _show_status("ERROR: " + str(msg), Color.RED))
		...
		# v146.0: Capturar error de socket inicial (Si el server no responde)
		if NetworkManager.has_signal("connection_failed"):
			NetworkManager.connection_failed.connect(func(): _show_status("SERVIDOR NO RESPONDE", Color.ORANGE))
