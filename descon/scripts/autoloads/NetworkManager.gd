...
signal aoi_entities_exited(data)
...
signal enemy_cast_started(data)
signal enemy_cast_ended(data)
signal enemy_cast_cancelled(data)
...
func _dispatch_event(e_name: String, e_data: Variant):
	match e_name:
		...
		"serverEnemyAuraTick": enemy_aura_tick.emit(e_data)
		...
		"aoiEntitiesExited": aoi_entities_exited.emit(e_data)
		...
		"enemyCastStart": enemy_cast_started.emit(e_data)
		"enemyCastEnd": enemy_cast_ended.emit(e_data)
		"enemyCastCancel": enemy_cast_cancelled.emit(e_data)
