...
module.exports = class AncientBossAI extends BaseAI {
    ...
    update(players, now, io) {
        ...
        if (!target) {
            ...
            if (now - this.noAggroStartTime > 10000) {
                if (this.enemy.hp < this.enemy.maxHp || this.enemy.shield < this.enemy.maxShield) {
                    console.log(`[BOSS-AI] Reset TOTAL Boss2 (Skills/HP/Rifts/Clones)`);
        ...
        // v239.11: Los clones NO entran en Rage, pero sigan moviéndose y atacando
        const isClone = this.enemy.name.toUpperCase().includes("CLONE");
        
        if (!isClone && !this.isRage && (now - this.combatStartTime > rageLimit)) {
            this.isRage = true;
        ...
        if (this.isRage) this.runRageMode(players, target, now, io);
    ...
    useClones(players, target, now, io) { 
        // v115.22: Evitar bucle infinito de clones (Solo el Boss original clona)
        if (this.enemy.name.toUpperCase().includes("CLONE")) return;

        if (global.serverSpawnEnemy) {
            // v239.06: Clones con daño reducido (10% del original) para evitar daño fantasma infinito
            global.serverSpawnEnemy(this.enemy.zone, 5, this.enemy.x+400, this.enemy.y+400, "Boss2 CLONE"); 
            
            // Buscar al clon recién creado y bajarle el daño agresivamente (v239.07)
            const clones = Object.values(global.enemies).filter(e => e.name === "Boss2 CLONE");
            clones.forEach(c => {
                c.hp = 1000; // Menos vida para morir rápido
                if (c.ai && c.ai.config) c.ai.config.bulletDamage = 50; 
                c.bulletDamage = 50;
            });
        }
    }
